...
export default function CommandCluster(props: CommandClusterProps) {
  ...
  if (DEBUG) {
    console.log(
      'CommandCluster runningCommand props:',
      props,
  ...
  React.useEffect(() => {
    if (runningCommandsRef.current.length > 0) {
      if (DEBUG) {
        console.log(
          'CommandCluster useEffect setting running command from runningCommandsRef',
          runningCommandsRef.current[runningCommandsRef.current.length - 1]
        );
