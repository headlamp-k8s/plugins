...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    // Empty data → leave compliance unset; the card renders "—" instead of a misleading 100%.
    const hasData = total > 0;
    const compliancePct = hasData ? Math.round((counts.pass / total) * 100) : null;
    ...
    <Box sx={{ p: 2 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid item xs={6} sm={3}>
            <MetricCard
              title={t('Compliance')}
              value={
                isLoading ? '...' : stats.compliancePct === null ? '—' : `${stats.compliancePct}%`
              }
              color={complianceColor}
            />
          </Grid>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Enforce')} value={isLoading ? '...' : stats.enforceCount} />
          </Grid>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Audit')} value={isLoading ? '...' : stats.auditCount} />
          </Grid>
        </Grid>

        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Pass')} value={stats.counts.pass} color="#4caf50" />
          </Grid>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Fail')} value={stats.counts.fail} color="#f44336" />
          </Grid>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Warn')} value={stats.counts.warn} color="#ff9800" />
          </Grid>
          <Grid item xs={6} sm={3}>
            <MetricCard title={t('Error')} value={stats.counts.error} color="#d32f2f" />
          </Grid>
        </Grid>
      </SectionBox>
      ...
      <Grid container spacing={2}>
        ...
        {categoryData.length > 0 && (
          <Grid item xs={12}>
            <SectionBox title={t('Top Failing Categories')}>
                ...
                {categoryData.map(({ name, value }) => {
                  ...
                  return (
                    <Box key={name} sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      <Typography
                        variant="body2"
                        sx={{ minWidth: 180, textAlign: 'right', flexShrink: 0 }}
                      >
                        {name}
                      </Typography>
                      <Box
        ...
        {!isLoading && !stats.hasData && (
          <Grid item xs={12}>
            <SectionBox title={t('No reports yet')}>
              <Typography variant="body2" color="text.secondary">
                {t(
                  'No policy results have been recorded for this cluster. Install some Kyverno policies to see compliance data here.'
                )}
              </Typography>
            </SectionBox>
          </Grid>
