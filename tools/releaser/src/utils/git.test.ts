...
describe('git utilities', () => {
  ...
  describe('getRepoRoot', () => {
    it('should return repo root from git command', () => {
      vi.mocked(execFileSync).mockReturnValue('/repo/root\n');
      expect(getRepoRoot()).toBe('/repo/root');
      expect(execFileSync).toHaveBeenCalledWith('git', ['rev-parse', '--show-toplevel'], expect.anything());
    });
  ...
  describe('commitPluginVersionChange', () => {
    it('should add files and commit', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['add', expect.stringContaining('package.json')], expect.anything());
      expect(execFileSync).toHaveBeenCalledWith('git', ['add', expect.stringContaining('package-lock.json')], expect.anything());
      expect(execFileSync).toHaveBeenCalledWith('git', ['commit', '--signoff', '-m', 'my-plugin: Bump version to 1.2.3'], expect.anything());
    });
  });
  ...
  describe('validateCommitSha', () => {
    ...
    it('should return false for invalid SHA', () => {
      vi.mocked(execFileSync).mockImplementation(() => { throw new Error('invalid sha'); });
  ...
  describe('createReleaseTag', () => {
    it('should create an annotated tag', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['tag', '-a', 'my-plugin-1.2.3', '-m', 'my-plugin 1.2.3'], expect.anything());
    });
  });
  ...
  describe('pushTag', () => {
    it('should push tag to origin', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['push', 'origin', 'my-plugin-1.2.3'], expect.anything());
    });
  });
  ...
  describe('checkGitStatus', () => {
    ...
    it('should return false if status is not empty', () => {
      vi.mocked(execFileSync).mockReturnValue('M file.txt');
  ...
  describe('getLatestTagForPlugin', () => {
    it('should return the newest tag', () => {
      vi.mocked(execFileSync).mockReturnValue('my-plugin-1.1.0\nmy-plugin-1.0.0\n');
  ...
  describe('hasChangesInPluginSinceTag', () => {
    it('should return true if diff has entries', () => {
      vi.mocked(execFileSync).mockReturnValue('modified-file.ts\n');
  ...
  describe('getCommitsSinceTag', () => {
    it('should return array of commit lines', () => {
      vi.mocked(execFileSync).mockReturnValue('hash1 commit message 1\nhash2 commit message 2\n');
  ...
  describe('tagExists', () => {
    it('should check local tag existence', () => {
      ...
      expect(execFileSync).toHaveBeenCalledWith('git', ['rev-parse', '--verify', 'my-tag'], expect.anything());
    });

    it('should check remote tag existence', () => {
      vi.mocked(execFileSync).mockReturnValue('');
      expect(tagExists('my-tag', true)).toBe(true);
      expect(execFileSync).toHaveBeenCalledWith('git', ['ls-remote', '--tags', 'origin', 'my-tag'], expect.anything());
    });
