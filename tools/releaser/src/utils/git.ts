...
export function commitPluginVersionChange(pluginName: string, version: string): void {
  ...
  try {
    execSync(`git add "${packageJsonPath}"`, { cwd: repoRoot });

    if (fs.existsSync(packageLockJsonPath)) {
      execSync(`git add "${packageLockJsonPath}"`, { cwd: repoRoot });
    }

    execSync(`git commit --signoff -m "${pluginName}: Bump version to ${version}"`, { cwd: repoRoot });
  } catch (error) {
...
export function getVersionBumpCommit(pluginName: string, version: string): string | null {
  try {
    ...
    const commits = execSync(`git log --format="%H" -S'"version": "${version}"' -- ${packageJsonPath}`, {
      encoding: 'utf-8',
      cwd: repoRoot
    }).trim();
    ...
    if (!commits) {
      ...
      const fallbackCommits = execSync(`git log --format="%H" -1 -- ${packageJsonPath}`, {
        encoding: 'utf-8',
        cwd: repoRoot
      }).trim();
...
export function validateCommitSha(commitSha: string): boolean {
  ...
  try {
    const repoRoot = getRepoRoot();
    execSync(`git rev-parse --verify ${commitSha}`, {
      cwd: repoRoot,
      stdio: 'pipe'
    });
    return true;
...
export function isCommitPushedToRemote(commitSha: string, remoteName: string = 'origin'): boolean {
  ...
  try {
    ...
    execSync(`git cat-file -e ${commitSha}`, {
      cwd: repoRoot,
      stdio: 'pipe'
    });

    // Now check if this commit is reachable from any remote branch
    const result = execSync(`git merge-base --is-ancestor ${commitSha} ${remoteName}/HEAD`, {
      cwd: repoRoot,
      stdio: 'pipe'
    });
...
export function createReleaseTag(pluginName: string, version: string): string {
  try {
    ...
    execSync(`git tag -a ${tagName} -m "${pluginName} ${version}"`, { cwd: repoRoot });
    return tagName;
  } catch (error) {
...
export function pushTag(tagName: string): void {
  try {
    const repoRoot = getRepoRoot();
    execSync(`git push origin ${tagName}`, { cwd: repoRoot });
  } catch (error) {
...
export function getChangelogForPlugin(pluginName: string): string {
  try {
    ...
    if (!latestTag) {
      ...
      const repoRoot = getRepoRoot();
      const allCommits = execSync(`git log --oneline HEAD -- ${pluginName}/`, {
        encoding: 'utf-8',
        cwd: repoRoot
      }).trim();
...
export function tagExists(tagName: string, checkRemote: boolean = false): boolean {
  try {
    ...
    if (checkRemote) {
      execSync(`git ls-remote --tags origin ${tagName}`, {
        cwd: repoRoot,
        stdio: 'ignore'
      });
    } else {
