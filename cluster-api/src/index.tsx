...
function registerClusterApiResource(config: ResourceRegistrationConfig) {
  ...
  registerRoute({
    ...
    sidebar: name,
    name: path.slice(0, -1), // Remove 's' from plural form
...
Object.assign(ResourceClasses, { Cluster });
Object.assign(ResourceClasses, { ClusterClass });
Object.assign(ResourceClasses, { KubeadmConfig });
Object.assign(ResourceClasses, { KubeadmConfigTemplate });
Object.assign(ResourceClasses, { KubeadmControlPlane });
Object.assign(ResourceClasses, { KubeadmControlPlaneTemplate });
Object.assign(ResourceClasses, { Machine });
Object.assign(ResourceClasses, { MachineDeployment });
Object.assign(ResourceClasses, { MachineDrainRule });
Object.assign(ResourceClasses, { MachineHealthCheck });
Object.assign(ResourceClasses, { MachinePool });
Object.assign(ResourceClasses, { MachineSet });
...
registerMapSource({
  ...
  useData() {
    const [clusterClasses] = ClusterClass.useList();
    const [clusters] = Cluster.useList();
    const [kubeadmConfigs] = KubeadmConfig.useList();
    const [kubeadmConfigTemplates] = KubeadmConfigTemplate.useList();
    const [kubeadmControlPlanes] = KubeadmControlPlane.useList();
    const [kubeadmControlPlaneTemplates] = KubeadmControlPlaneTemplate.useList();
    const [machines] = Machine.useList();
    const [machineDeployments] = MachineDeployment.useList();
    const [machineDrainRules] = MachineDrainRule.useList();
    const [machineHealthChecks] = MachineHealthCheck.useList();
    const [machinePools] = MachinePool.useList();
    const [machineSets] = MachineSet.useList();

    // Array of resource lists and their detail components
    const resourceLists = [
      { items: clusterClasses, detailsComponent: ClusterClassDetail },
      { items: clusters, detailsComponent: ClusterDetail },
      { items: kubeadmConfigs, detailsComponent: KubeadmConfigDetail },
      { items: kubeadmConfigTemplates, detailsComponent: KubeadmConfigTemplateDetail },
      { items: kubeadmControlPlanes, detailsComponent: KubeadmControlPlaneDetail },
      { items: kubeadmControlPlaneTemplates, detailsComponent: KubeadmControlPlaneTemplateDetail },
      { items: machines, detailsComponent: MachineDetail },
      { items: machineDeployments, detailsComponent: MachineDeploymentDetail },
      { items: machineDrainRules, detailsComponent: MachineDrainRuleDetail },
      { items: machineHealthChecks, detailsComponent: MachineHealthCheckDetail },
      { items: machinePools, detailsComponent: MachinePoolDetail },
      { items: machineSets, detailsComponent: MachineSetDetail },
    ];
    ...
    return useMemo(() => {
      ...
      const edges = [];
      // Add edges between any nodes that have an owner reference
