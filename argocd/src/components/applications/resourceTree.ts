...
/**
 * Inline SVG of the Argo CD octopus logo rendered as a React element.
 * Uses the Simple Icons "argo" glyph path. No external icon library needed.
 */
const argoOctopusSvg = React.createElement(
  'svg',
  {
...
export function buildArgoCDProjectGraph(
  ...
  const nodes = new Map<string, GraphNode>();
  const edges = new Map<string, GraphEdge>();
  const statusPriority = { success: 1, warning: 2, error: 3 } as const;
  ...
  for (const project of projects) {
    ...
    const projectStatus = projectApplications.reduce<GraphNode['status']>(
      (current, application) => {
        const applicationStatus = getGraphStatus(application.syncStatus, application.healthStatus);
        if (!applicationStatus) return current;
        if (!current || statusPriority[applicationStatus] > statusPriority[current]) {
