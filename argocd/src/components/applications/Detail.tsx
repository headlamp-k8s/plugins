...
function SourceSummary(props: {
  ...
  const repoHref = source?.repoURL ? getSafeExternalUrl(source.repoURL) : null;
    ...
    <Box sx={{ display: 'grid', gap: 0.5 }}>
      ...
      <SummaryRow
        ...
        value={
          source?.repoURL ? (
            repoHref ? (
              <MuiLink href={repoHref} target="_blank" rel="noopener noreferrer">
...
function getSyncPolicySection(app: ArgoApplication) {
  ...
  const rows = [
    { name: 'Retry Limit', value: policy?.retry?.limit ?? 'Default' },
    {
      name: 'Retry Backoff',
      value: backoff
        ? `${backoff.duration ?? 'Default'} (max ${backoff.maxDuration ?? 'Default'})`
        : 'Default',
    },
    { name: 'Retry Factor', value: backoff?.factor ?? 'Default' },
    {
      name: 'Sync Options',
