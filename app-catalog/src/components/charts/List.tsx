...
const useAppCatalogConfig = store.useConfig();
...
export function ChartsList({ fetchCharts = fetchChartsFromArtifact }) {
  ...
  // note: since we default to true for showVerified and the settings page is not accessible from anywhere else but the list comp
  // we must have the default value here and have it imported for use in the settings tab
  const config = useAppCatalogConfig() as { showVerified: boolean };
  const showVerified = config?.showVerified ?? true;
  ...
  useEffect(
    function fetchChartsOnPageChange() {
      ...
      async function fetchData() {
        try {
          const response: any = await fetchCharts(search, showVerified, chartCategory, page);
    ...
    [page, chartCategory, search, showVerified]
  );
