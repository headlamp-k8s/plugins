...
const clusterName = process.env.E2E_CLUSTER_NAME || 'kmesh-e2e';
const headlampUrl = process.env.HEADLAMP_URL || 'http://127.0.0.1:4467';
const headlampPort = new URL(headlampUrl).port || '4467';
...
async function main(): Promise<void> {
  ...
  run('kubectl', ['apply', '-f', 'e2e/gateway-crd.yaml']);
  run('kubectl', ['apply', '-f', 'e2e/kmesh-crds.yaml']);
  run('kubectl', [
    'wait',
  ...
  run('docker', ['build', '-f', 'e2e/Dockerfile.headlamp', '-t', 'headlamp-kmesh-e2e:local', '.']);
