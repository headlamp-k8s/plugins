...
|---|---|---|
| Headlamp | 0.44.0 or later | The Knative menu uses sidebar section headers introduced in Headlamp 0.44.0. |
...
The sidebar groups the views into three sections: **Serving** for the resources application
and domain owners manage, **Serving Internals** for internal resources, including the read-only
resources Serving derives and manages itself, and **Configuration** for cluster-level Knative
settings.
...
Edit, Delete, Scale, and Restart actions are hidden for those exact Knative resource kinds.
Cluster Domain Claims sit with the other internal resources but always show their create
action, so claims can be created manually on clusters where
