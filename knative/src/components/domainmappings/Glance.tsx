...
/**
 * Renders a brief "glance" summary for a DomainMapping on the map.
 * Displays the target reference and ready URL.
 *
 * @param props - Component properties.
 * @param props.node - The map node containing the resource.
 */
export function DomainMappingGlance({ node }: GlanceProps) {
  if (node.kubeObject?.kind === KnativeDomainMapping.kind) {
    const dm = node.kubeObject as KnativeDomainMapping;
    const readyCond = dm.status?.conditions?.find(c => c.type === 'Ready');
    const readyStatus = readyCond?.status || 'Unknown';

    return (
      <Box display="flex" gap={1} alignItems="center" mt={2} flexWrap="wrap" key="dm-glance">
        <StatusLabel
