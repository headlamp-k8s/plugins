...
const knativeServiceSource: any = {
  id: 'knative-service',
  label: 'KService',
  icon: <Icon icon="simple-icons:knative" width="100%" height="100%" color="rgb(7, 102, 174)" />,
  useData() {
  ...
  useData() {
    ...
    return useMemo(() => {
      ...
      const nodes = revisions.map(rev => {
        let traffic: any[] = [];
        if (kservices && rev.parentService) {
          const svc = kservices.find(
            s =>
              s.metadata.name === rev.parentService &&
              s.metadata.namespace === rev.metadata.namespace
          );
          if (svc) {
            traffic = rev.getTrafficInService(svc);
          }
        }
      ...
      revisions.forEach(rev => {
        if (kservices && rev.parentService) {
          ...
          if (svc) {
            edges.push(makeKubeToKubeEdge(svc, rev));
          }
        }
      });
...
const knativeDomainMappingSource: any = {
  ...
  useData() {
    ...
    return useMemo(() => {
      ...
      domainMappings.forEach(dm => {
        if (kservices && dm.spec?.ref?.kind === 'Service') {
          const svc = kservices.find(
            s =>
              s.metadata.name === dm.spec.ref.name && s.metadata.namespace === dm.metadata.namespace
          );
          if (svc) {
            edges.push(makeKubeToKubeEdge(dm, svc));
          }
        }
      });
