...
function createDesktopApi(initialToolsConfig = sampleToolsConfig) {
  let toolsConfig = JSON.parse(JSON.stringify(initialToolsConfig));
  ...
  return {
    mcp: {
      ...
      updateConfig: async (config: any) => ({ success: true, config }),
      getToolsConfig: async () => ({ success: true, config: toolsConfig }),
      updateToolsConfig: async (config: any) => {
        toolsConfig = JSON.parse(JSON.stringify(config));
        return { success: true };
      },
...
const Template: StoryFn<React.ComponentProps<typeof ToolsDialog>> = args => {
  ...
  if (typeof window !== 'undefined') {
    (window as any).desktopApi = createDesktopApi();
  }
