...
const azureSkills: SkillDisplayInfo[] = [
  {
...
const k8sSkills: SkillDisplayInfo[] = [
  {
    name: 'pod-debugging',
...
const localSkills: SkillDisplayInfo[] = [
  {
  ...
  loadSkills: async () => {
    ...
    for (const name of names) {
      skills.push({
        ...
        contentSizeBytes: 500 + Math.floor(Math.random() * 5000),
      });
    }
    // Add some from another repo
    for (const name of ['pod-debug', 'service-mesh', 'helm-charts']) {
      skills.push({
        name,
        description: `Kubernetes ${name} guidance.`,
        source: `https://github.com/k8s-community/skills@def456/${name}/SKILL.md`,
        content: `# ${name}\n\nGuidance for ${name}.`,
        contentSizeBytes: 200 + Math.floor(Math.random() * 1000),
      });
    }
    return skills;
  },
