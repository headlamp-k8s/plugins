...
export interface AIAssistantToggleProps {
  ...
  isOpen: boolean;
  /** Callback invoked when the user clicks the toggle button. */
  onToggle: () => void;
  /** Whether to show the "configure your provider" popover. */
  showConfigPrompt: boolean;
  /** Callback invoked when the user dismisses the configuration popover. */
  onDismissPrompt: () => void;
  /** Callback invoked when the user clicks "Open Settings" in the popover. */
  onConfigure: () => void;
    ...
    <Box sx={{ display: 'flex', alignItems: 'center' }}>
      <Tooltip title={effectiveTooltipTitle}>
        <ToggleButton
          ...
          value="ai-assistant"
        >
          <Icon icon={icon} width="24px" color={theme.palette.text.primary} />
        </ToggleButton>
      </Tooltip>
