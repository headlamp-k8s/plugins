...
import type { SkillsConfig } from '@headlamp-k8s/ai-common/skills/SkillConfigManager';
import { DEFAULT_SKILLS_CONFIG } from '@headlamp-k8s/ai-common/skills/SkillConfigManager';
...
export function parseSkillSourceUrl(url: string): SkillSource {
  ...
  if (!isValidGitUrl(repoUrl)) {
    throw new Error(
      `Invalid skill source URL: ${repoUrl}. Only HTTPS URLs to github.com, gitlab.com, or bitbucket.org are allowed.`
    );
  }
