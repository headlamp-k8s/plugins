...
describe('promptLinkHelper', () => {
  describe('promptLinksInstructions', () => {
    it('is a non-empty string', () => {
      expect(typeof promptLinksInstructions).toBe('string');
      expect(promptLinksInstructions.length).toBeGreaterThan(0);
    });

    it('contains resource linking instructions', () => {
      expect(promptLinksInstructions).toContain('RESOURCE LINKING');
    });

    it('contains the headlamp resource-details link pattern', () => {
      expect(promptLinksInstructions).toContain('headlamp/resource-details');
    });

    it('contains the headlamp cluster link pattern', () => {
      expect(promptLinksInstructions).toContain('headlamp/cluster');
    });

    it('does not require window or browser APIs (is Node-safe)', () => {
      // This test just verifies the import works in a Node context (vitest)
      expect(promptLinksInstructions).toBeDefined();
    });
  });
});
