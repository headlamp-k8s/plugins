...
import {
  DEMO_CLUSTER_EXPLORATION,
  DIAGNOSIS_FIXTURES,
  GENERAL_FIXTURES,
} from './builtinFixtures.js';
 ...
 */
export interface MockTestingModelOptions {
 ...
 */
export function createMockTestingModel(options: MockTestingModelOptions = {}): BaseChatModel {
  ...
  function patchGenerate(m: any) {
    ...
    m._generate = async function (messages: any, options: any, runManager: any) {
      let matched: string;
      ...
      if (sequenceTurns) {
        ...
        const lastHuman = [...messages].reverse().find((msg: any) => msg._getType() === 'human');
        const input =
