  ...
  private sanitizeToolAlignment(prompts: Prompt[]): Prompt[] {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      if (p.role === 'assistant' && p.toolCalls && p.toolCalls.length > 0) {
        ...
        const matchedCalls = p.toolCalls.filter((tc: any) => toolResultIds.has(tc.id));
  ...
  private async executeSingleTool(tool: RecommendedTool): Promise<any> {
    try {
      ...
      return {
        ...
        toolName: tool.name,
        error: true,
        message: (error as Error | null)?.message || 'Unknown error occurred',
      };
    }
  }
...
${Object.entries(toolResults)
  ...
  private async handleToolCalls(response: any): Promise<Prompt> {
    ...
    const allToolCalls = response.tool_calls.map((tc: any) => ({
      type: 'function',
      id: tc.id,
      function: {
        name: tc.name,
    ...
    const toolCalls = allToolCalls.filter((tc: any) => enabledToolIds.includes(tc.function.name));

    // Log if any tools were filtered out
    const filteredOutTools = allToolCalls.filter(
      (tc: any) => !enabledToolIds.includes(tc.function.name)
    );
    ...
    if (toolCalls.length === 0) {
      ...
      if (filteredOutTools.length > 0) {
        const disabledToolNames = filteredOutTools.map((tc: any) => tc.function.name).join(', ');
...
Without access to the Kubernetes API, I cannot fetch current pod, deployment, service, or other resource information from your cluster.`;
    ...
    try {
      ...
      const approvedToolCalls = toolCalls
        .filter((tc: any) => approvedToolIds.includes(tc.id))
        .map((tc: any) => {
      ...
      const deniedToolCalls = toolCalls.filter((tc: any) => !approvedToolIds.includes(tc.id));
    ...
    const toolResponses = this.history.filter(
      prompt => prompt.role === 'tool' && toolCalls.some((tc: any) => tc.id === prompt.toolCallId)
    );
  ...
  private async handleToolCallsForStreaming(
    ...
    const enabledToolCalls = formattedToolCalls.filter((tc: any) =>
    ...
    try {
      ...
      // Add denied tool responses to history
      const deniedToolCalls = enabledToolCalls.filter(
        (tc: any) => !approvedToolIds.includes(tc.id)
      );
      for (const deniedTool of deniedToolCalls) {
        this.history.push({
          role: 'tool',
...
Format your response to make the errors prominent and actionable.`,
  ...
  private validateToolCallAlignment(): void {
    ...
    if (lastAssistantMessage?.toolCalls) {
      const expectedToolCallIds = lastAssistantMessage.toolCalls.map((tc: any) => tc.id);
  ...
  private async handleToolResponseResult(response: any): Promise<Prompt> {
    ...
    //         correctedResponse.tool_calls.map((tc: any) => ({ name: tc.name, args: tc.args }))
    //       );
    ...
    const assistantPrompt: Prompt = {
      ...
      toolCalls:
        correctedResponse.tool_calls?.map((tc: any) => ({
          id: tc.id,
          type: 'function',
          function: {
