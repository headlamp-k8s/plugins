...
export function generateContextDescription(
  ...
  if (event?.items && Array.isArray(event.items)) {
    ...
    if (itemCount > 0) {
      ...
      if (resourceType === 'Pod') {
        const unhealthyPods = event.items.filter((pod: any) => {
  ...
  if (event?.resources && Array.isArray(event.resources)) {
    ...
    if (resourceCount > 0) {
      ...
      if (resourceType === 'Pod') {
        const unhealthyPods = event.resources.filter((resource: any) => {
        ...
        const unhealthyDeployments = event.resources.filter((resource: any) => {
