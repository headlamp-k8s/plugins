...
import {
  createMockTestingAgent,
  type MockTestingAgent,
  type MockTestingAgentOptions,
} from '../mock-testing-agent/MockTestingAgent';
...
export class MockHolmesAgent {
  private mockAgent: MockTestingAgent;
  ...
  constructor(options?: MockTestingAgentOptions) {
    this.mockAgent = createMockTestingAgent({
  ...
  async runAgent(_?: {
    ...
    try {
      ...
      for (const sub of this.subscribers) {
        sub.onRunErrorEvent?.({ event: { message: err?.message || 'Mock agent error' } });
        sub.onRunFinalized?.();
      }
    }
  }
