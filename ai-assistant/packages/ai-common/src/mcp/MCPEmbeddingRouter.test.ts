...
import { cosineSimilarity, MCPEmbeddingRouter } from './MCPEmbeddingRouter';
import { MCPToolInfo } from './MCPToolRouter';
...
describe('MCPEmbeddingRouter', () => {
  ...
  describe('indexTools', () => {
    it('indexes tools successfully', async () => {
      const router = new MCPEmbeddingRouter(new FakeEmbeddings());
    ...
    it('falls back gracefully on embedding failure', async () => {
      const router = new MCPEmbeddingRouter(new FailingEmbeddings());
