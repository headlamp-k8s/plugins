 ...
 */
export interface ScoredMCPTool {
  /** The tool info. */
  tool: MCPToolInfo;
 ...
 */
export interface MCPToolRouterConfig {
...
export const DEFAULT_MCP_ROUTER_CONFIG: MCPToolRouterConfig = {
  maxTools: 10,
  minScore: 0.1,
};
 ...
 * keyword matching.
 */
export function buildToolSearchText(tool: MCPToolInfo): string {
  const parts: string[] = [];
...
export function routeMCPTools(
  ...
  const scored: ScoredMCPTool[] = tools.map(tool => {
 ...
 */
export function scoreMCPTools(query: string, tools: MCPToolInfo[]): ScoredMCPTool[] {
  const queryTokens = tokenize(query);
