...
describe('MCPToolRouter', () => {
  ...
  describe('routeMCPTools', () => {
    ...
    it('returns all tools when count is below maxTools', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 20 };
      const result = routeMCPTools('anything', tools, config);
      expect(result).toHaveLength(11);
    });

    it('selects kubectl tools for pod-related query', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 3 };
      const result = routeMCPTools('list all pods in the kube-system namespace', tools, config);
    ...
    it('selects helm tools for helm-related query', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 3 };
      const result = routeMCPTools('install a helm chart release', tools, config);
      const names = result.map(t => t.name);
    ...
    it('selects docker tools for docker-related query', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 3 };
      const result = routeMCPTools('build and push a docker image', tools, config);
      const names = result.map(t => t.name);
    ...
    it('selects terraform tools for infra query', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 3 };
      const result = routeMCPTools('terraform plan apply infrastructure', tools, config);
      const names = result.map(t => t.name);
      expect(names).toContain('terraform__plan');
    });

    it('respects maxTools limit', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 2 };
      const result = routeMCPTools('kubernetes', tools, config);
      expect(result.length).toBeLessThanOrEqual(2);
    });

    it('respects minScore threshold', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 5, minScore: 0.95 };
      const result = routeMCPTools('completely unrelated foobar xyz', tools, config);
      expect(result.length).toBe(0);
    });

    it('returns up to maxTools for empty query', () => {
      const config = { ...DEFAULT_MCP_ROUTER_CONFIG, maxTools: 3 };
      const result = routeMCPTools('', tools, config);
