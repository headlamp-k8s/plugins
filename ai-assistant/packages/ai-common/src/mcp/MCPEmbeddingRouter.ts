 ...
 *
 * Design mirrors {@link EmbeddingRouter} for skills:
 ...
 */
export class MCPEmbeddingRouter {
   ...
   */
  async indexTools(tools: MCPToolInfo[]): Promise<void> {
    if (tools.length === 0) {
      this.embeddedTools = [];
      this.isIndexed = true;
      return;
   ...
   *
   * Falls back to keyword-based {@link routeMCPTools} when the index is
   * unavailable or the query embedding fails.
   */
  async route(
    query: string,
    tools: MCPToolInfo[],
    config: MCPToolRouterConfig = DEFAULT_MCP_ROUTER_CONFIG
  ): Promise<MCPToolInfo[]> {
    ...
    if (!this.isIndexed || this.embeddedTools.length === 0) {
      return routeMCPTools(query, tools, config);
    }
   ...
   */
  async scoreTools(query: string): Promise<ScoredMCPTool[]> {
    if (!this.isIndexed || this.embeddedTools.length === 0) {
      return [];
    }
    ...
    try {
      ...
      const scored: ScoredMCPTool[] = this.embeddedTools.map(({ tool, vector }) => ({
        tool,
  ...
  private rankBySimilarity(queryVector: number[], config: MCPToolRouterConfig): MCPToolInfo[] {
