...
import { buildSkillEmbeddingText, cosineSimilarity, EmbeddingRouter } from './EmbeddingRouter';
import { ParsedSkill } from './skillParser';
import { DEFAULT_ROUTER_CONFIG, SkillRouterConfig } from './SkillRouter';
...
function createMockEmbeddings(opts?: {
  ...
  } as unknown as Embeddings & { vocabulary: string[] };
...
describe('EmbeddingRouter', () => {
  ...
  beforeEach(() => {
    ...
    router = new EmbeddingRouter(mockEmbeddings);
  });
  ...
  describe('indexSkills', () => {
    ...
    it('handles embedding failure gracefully', async () => {
      ...
      const failRouter = new EmbeddingRouter(failingEmbeddings);
