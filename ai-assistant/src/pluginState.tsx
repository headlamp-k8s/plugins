...
import {
  ...
  initializeToolsState as discoverInitialToolsState,
...
const loadMCPToolsConfig: MCPToolCatalogLoader = async () => {
  if (typeof window === 'undefined' || !window.desktopApi?.mcp) return { success: false };
  return window.desktopApi.mcp.getToolsConfig();
};

export function getAllAvailableToolsIncludingMCP() {
  return discoverAllAvailableToolsIncludingMCP(loadMCPToolsConfig);
}

export function getEnabledToolIdsIncludingMCP(pluginSettings: unknown) {
  return discoverEnabledToolIdsIncludingMCP(pluginSettings, loadMCPToolsConfig);
}

export function isMCPTool(toolName: string) {
  return discoverMCPTool(toolName, loadMCPToolsConfig);
}

export function initializeToolsState(pluginSettings: unknown) {
  return discoverInitialToolsState(pluginSettings, loadMCPToolsConfig);
}
...
export {
  ...
  isBuiltInTool,
} from '@headlamp-k8s/ai-common/tools/catalog/toolDefinitions';
export { parseMCPToolName } from '@headlamp-k8s/ai-common/mcp/tools/toolName';
export { isToolEnabled, toggleTool } from '@headlamp-k8s/ai-common/tools/settings/enabledTools';
