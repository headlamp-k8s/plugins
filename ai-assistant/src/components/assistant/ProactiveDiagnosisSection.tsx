...
function getStepIcon(type: DiagnosisThinkingStep['type']): string {
  switch (type) {
    case 'tool-start': return 'mdi:wrench';
    case 'tool-result': return 'mdi:check';
    case 'todo-update': return 'mdi:format-list-checks';
    default: return 'mdi:message-text-outline';
  }
}

function getStepIconColor(type: DiagnosisThinkingStep['type'], theme: any): string {
  switch (type) {
    case 'tool-start': return theme.palette.info.main;
    case 'tool-result': return theme.palette.success.main;
    case 'todo-update': return theme.palette.warning.main;
...
function getStepTypeLabel(type: DiagnosisThinkingStep['type']): string {
  switch (type) {
    ...
    case 'todo-update': return 'Plan update';
...
function splitDiagnosisContent(text: string): { thinking: string; answer: string } {
  ...
  const isThinkingLine = (line: string) =>
    thinkingPatterns.some(p => p.test(line.trim()));
...
function DiagnosisResultBlock({ diagnosis, onYamlAction }: { diagnosis: DiagnosisResult; onYamlAction?: (yaml: string, title: string, resourceType: string, isDelete: boolean) => void }) {
  const theme = useTheme();
    ...
    <Box sx={{ mt: 0.5, mb: 1, ml: 3 }}>
      ...
      {stepsToShow.length === 0 && thinking && (
        <CollapsibleSection
          ...
          subtitle={`${(thinking.match(/🔧/g) || []).length} tool call${(thinking.match(/🔧/g) || []).length !== 1 ? 's' : ''}`}
        >
          <Typography
            variant="caption"
            component="pre"
      ...
      {answer && (
        ...
        >
          ...
          >
            <ContentRenderer content={answer} onYamlDetected={onYamlAction ? handleYamlDetected : undefined} />
          </Box>
        </CollapsibleSection>
      )}
      ...
      {!answer && !thinking && (
        ...
        >
          ...
          >
            <ContentRenderer content={diagnosis.diagnosis} onYamlDetected={onYamlAction ? handleYamlDetected : undefined} />
          </Box>
        </CollapsibleSection>
      )}
    </Box>
        ...
        >
          ...
          >
            ...
            <Icon
              ...
              color={d.event.type === 'Error' ? theme.palette.error.main : theme.palette.warning.main}
              width={16}
              style={{ flexShrink: 0 }}
            />
            <Typography
