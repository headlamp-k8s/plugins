...
vi.mock('@headlamp-k8s/ai-common/managers/ToolConfigManager', () => ({
  getAllAvailableToolsIncludingMCP: async () => [],
  initializeToolsState: async () => [],
}));

vi.mock('@headlamp-k8s/ai-common/managers/ProviderConfigManager', () => ({}));
...
describe('setIsUIPanelOpen — does not persist to store', () => {
  ...
  it('does not call pluginStore.update when panel is opened', () => {
    const { result } = renderHook(() => useGlobalState());
  ...
  it('updates React state when panel is opened', () => {
    const { result } = renderHook(() => useGlobalState());
