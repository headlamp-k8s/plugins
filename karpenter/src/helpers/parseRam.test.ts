...
describe('parseRam', () => {
  ...
  it('treats the K/M/G/T suffixes the same as their Ki/Mi/Gi/Ti forms', () => {
    expect(parseRam('1K')).toBe(parseRam('1Ki'));
    expect(parseRam('1M')).toBe(parseRam('1Mi'));
    expect(parseRam('1G')).toBe(parseRam('1Gi'));
    expect(parseRam('1T')).toBe(parseRam('1Ti'));
  });
