...
/**
 * Props for the CapiChart component.
 * @interface CapiChartProps
 * @property {ChartConfig[]} chartConfigs - List of available chart configurations for this resource.
 * @property {string} [defaultChart] - Optional key of the chart to select by default.
 */
interface CapiChartProps {
...
/**
 * A generic chart container for Cluster API metrics.
 * Handles Prometheus state checking, interval/resolution selection, and
 * switching between different chart variants (e.g., Reconcile Rate vs Duration).
 */
export function CapiChart(props: CapiChartProps) {
...
/**
 * Generates X-axis props for recharts with proper timestamp formatting.
 * @param timespan - The current chart timespan.
 * @param theme - The MUI theme for styling.
 * @returns Object containing dataKey, tickLine, and tick renderer.
 */
export function getChartXAxisProps(timespan: string, theme: any) {
  ...
  return {
    ...
    tick: (tickProps: any) => {
      ...
      return (
        ...
        >
          <text x={0} y={10} dy={0} textAnchor="middle" style={{ fontSize: '11px' }}>
            {value}
          </text>
        </g>
      );
