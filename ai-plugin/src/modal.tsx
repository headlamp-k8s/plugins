...
import { Drawer, Box, Chip, FormControl, InputLabel, OutlinedInput, InputAdornment, Button, Divider, TextField } from '@material-ui/core';
...
export default function AIPrompt(props: {
  ...
  function preparePrompt(event, promptValue) {
    ...
    if (resource) {
      ...
      if(promptVal.toLowerCase().includes(`Explain ${resource.kind}`.toLowerCase())) {
        ...
        return prompt.base_prompt + "Q: " + promptValue + " " + "The complete resource json for this resource is "+ JSON.stringify(resource) + "\n";
      }
    ...
    if(items[0].kind) {
      return prompt.base_prompt + "Q: " + promptValue + "The items are "+ items.map((item) => {
        return `kind: ${item.kind} name: ${item.metadata.name} ${item.metadata.namespace || ""} ${item.message || ""}\n`
      }) + "\n";
    }
    if(items) {
      return prompt.base_prompt + "Q: " + promptValue + "The items are "+ JSON.stringify(items) + "\n";
    }
  ...
  if(_pluginSetting.event?.resource) {
    const finalPrompts = promptHelpers.details_view_loaded_with_resource.map(prompt => {
      return formatString(prompt, _pluginSetting.event?.resource.kind, _pluginSetting.event?.resource.metadata.name);
    })
    availablePrompts = availablePrompts.concat(finalPrompts)
    if(_pluginSetting.event?.resource.kind === 'Deployment') {
      availablePrompts = availablePrompts.concat(promptHelpers.Deployment.map(prompt => {
  ...
  if(_pluginSetting.event?.items?.[0]?.kind) {
    const finalPrompts = promptHelpers.list_view_loaded_with_resource.concat(promptHelpers.list_view_loaded_without_resource).map((prompt) => {
  ...
  return openPopup ? (
    <div ref={rootRef}>
       <Drawer open={openPopup} anchor={'right'} 
       transitionDuration={{enter: 2000, exit: 2000}}
       BackdropProps={{ invisible: true }}
       variant="persistent"
       PaperProps={{
        style: { flexDirection: "column-reverse", padding: '1rem', width: '25%', maxWidth: '50%', height: '95vh', top: '6.7vh'}}}>
       <Box display="flex" mb={2} flexWrap="wrap"  justifyContent="flex-end" flexDirection="column-reverse">
        <Box>
          <TextField
