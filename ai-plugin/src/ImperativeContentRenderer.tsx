...
const ImperativeContentRenderer: React.FC<ImperativeContentRendererProps> = ({
  ...
  const yamlContainers = useMemo(() => new Map<string, { container: HTMLDivElement; root: Root }>(), []);
  ...
  const renderYamlImperatively = useCallback((yaml: string, resourceType: string, containerId: string) => {
            ...
            <div
              ref={(element) => {
                if (element && !element.hasChildNodes()) {
                  ...
                  const yamlContainer = renderYamlImperatively(children, parsed.resourceType, yamlId);
...
export default React.memo(ImperativeContentRenderer, (prevProps, nextProps) => {
  return prevProps.content === nextProps.content && prevProps.onYamlDetected === nextProps.onYamlDetected;
});
