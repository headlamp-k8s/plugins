 ...
 */
export const useImperativeYamlRender = ({
  theme,
  onYamlDetected,
}: UseImperativeYamlRenderOptions) => {
  const containerRegistry = useRef<Map<string, { container: HTMLDivElement; root: Root }>>(
    new Map()
  );
  const lastContentRegistry = useRef<Map<string, { yaml: string; resourceType: string }>>(
    new Map()
  );

  const renderYamlImperatively = useCallback(
    (containerId: string, yaml: string, resourceType: string): HTMLDivElement => {
  ...
  const cleanupAllContainers = useCallback(() => {
    containerRegistry.current.forEach(entry => {
      entry.root.unmount();
    });
