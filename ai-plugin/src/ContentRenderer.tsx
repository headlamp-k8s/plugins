...
import IsolatedYamlRenderer from './IsolatedYamlRenderer';
...
const ContentRenderer: React.FC<ContentRendererProps> = ({ content, onYamlDetected }) => {
  ...
  const processUnformattedYaml = useCallback((content: string) => {
    ...
    parts.forEach((part, index) => {
      ...
      if (isYamlPart) {
        ...
        // Regular text content - use ReactMarkdown with memoized components
        sections.push(
          <ReactMarkdown
  ...
  }, [onYamlDetected, markdownComponents]);
  ...
  }, [content, onYamlDetected, markdownComponents, processUnformattedYaml]);
...
const arePropsEqual = (prevProps: ContentRendererProps, nextProps: ContentRendererProps) => {
  return (
...
export default React.memo(ContentRenderer, arePropsEqual);
