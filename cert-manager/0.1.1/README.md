...
- **Cloudflare DNS01 solver support in the ACME challenge view.** The `ACMEChallengeSolverComponent` now renders Cloudflare-specific configuration — account email, API key secret reference, and API token secret reference — when an Issuer or ClusterIssuer uses Cloudflare as its DNS01 provider. Visible on both Challenge and Issuer detail pages.
