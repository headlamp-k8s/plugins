...
const HelmRelease = makeCustomResourceClass({
  apiInfo: [{ version: 'v2', group: 'helm.toolkit.fluxcd.io' }],
  isNamespaced: true,
  singularName: 'helmrelease',
  pluralName: 'helmreleases',
});

const HelmRepostory = makeCustomResourceClass({
  apiInfo: [{ version: 'v1', group: 'source.toolkit.fluxcd.io' }],
  isNamespaced: true,
  singularName: 'helmrepository',
  pluralName: 'helmrepositories',
});

const OciSource = makeCustomResourceClass({
  apiInfo: [{ version: 'v1beta2', group: 'source.toolkit.fluxcd.io' }],
  isNamespaced: true,
  singularName: 'ocirepository',
  pluralName: 'ocirepositories',
});

const Kustomization = makeCustomResourceClass({
  apiInfo: [{ version: 'v1', group: 'kustomize.toolkit.fluxcd.io' }],
  isNamespaced: true,
...
const helmReleaseSource = {
  ...
  icon: <Icon icon="simple-icons:flux" width="100%" height="100%" color="rgb(50, 108, 229)" />,
  useData() {
    const [deployments] = Deployment.default.useList();
    const [releases] = HelmRelease.useList();
