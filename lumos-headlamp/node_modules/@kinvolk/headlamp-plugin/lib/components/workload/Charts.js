...
export function WorkloadCircleChart(props) {
    ...
    const [total, partial] = useMemo(() => {
        ...
        const partial = workloadData?.filter(item => getReadyReplicas(item) !== getTotalReplicas(item)).length || 0;
    ...
    return (_jsx(TileChart, { data: makeData(), total: total, totalProps: {
            fill: theme.palette.chartStyles.fillColor || theme.palette.common.black,
        }, label: getLabel(), legend: getLegend(), ...other }));
}
