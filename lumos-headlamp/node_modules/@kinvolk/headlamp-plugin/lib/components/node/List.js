...
export default function NodeList() {
    ...
    return (_jsx(ResourceListView, { title: t('Nodes'), headerProps: {
            noNamespaceFilter: true,
        }, resourceClass: Node, columns: [
            'name',
            'cluster',
            {
                id: 'cpu',
                ...
                render: node => (_jsx(UsageBarChart, { node: node, nodeMetrics: nodeMetrics, resourceType: "cpu", noMetrics: noMetrics })),
            },
            {
                id: 'memory',
                label: t('Memory'),
                ...
                render: node => (_jsx(UsageBarChart, { node: node, nodeMetrics: nodeMetrics, resourceType: "memory", noMetrics: noMetrics })),
            },
            {
                id: 'ready',
                ...
                render: node => _jsx(NodeReadyLabel, { node: node }),
            },
            {
                id: 'taints',
                label: t('translation|Taints'),
                getValue: node => node.spec?.taints?.map(taint => `${taint.key}:${taint.effect}`)?.join(', '),
                render: (item) => _jsx(NodeTaintsLabel, { node: item }),
            },
            {
                id: 'roles',
                label: t('Roles'),
                ...
                getValue: node => {
                    return Object.keys(node.metadata.labels ?? {})
                        .filter((t) => t.startsWith('node-role.kubernetes.io/'))
            ...
            },
            {
                ...
                render: node => {
                    ...
                    return (_jsxs(_Fragment, { children: [_jsx(HoverInfoLabel, { label: node.status.nodeInfo.osImage, hoverInfo: t('OS image'), labelProps: { variant: 'body2' }, iconPosition: "start", icon: osIcon }), node.status.nodeInfo.kernelVersion && (_jsx(HoverInfoLabel, { label: node.status.nodeInfo.kernelVersion, hoverInfo: t('Kernel version'), labelProps: { variant: 'body2' }, iconPosition: "start", icon: "mdi:nut" })), _jsx(HoverInfoLabel, { label: node.status.nodeInfo.containerRuntimeVersion, hoverInfo: t('Container Runtime'), labelProps: { variant: 'body2' }, iconPosition: "start", icon: "mdi:train-car-container" })] }));
                },
                show: false,
            },
            'age',
