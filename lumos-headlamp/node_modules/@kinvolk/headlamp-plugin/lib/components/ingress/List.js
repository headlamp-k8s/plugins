...
function RulesDisplay(props) {
    ...
    return _jsx(LabelListItem, { labels: rulesText });
}
export default function IngressList() {
    const { t } = useTranslation(['glossary', 'translation']);
    return (_jsx(ResourceListView, { title: t('Ingresses'), resourceClass: Ingress, columns: [
            'name',
            'namespace',
            'cluster',
            {
                ...
                render: ingress => ingress.spec?.ingressClassName ? (_jsx(Link, { routeName: "ingressclass", params: { name: ingress.spec?.ingressClassName }, children: ingress.spec?.ingressClassName })) : null,
            },
            {
                id: 'hosts',
                label: t('Hosts'),
                getValue: ingress => ingress
                    ...
                    .map(r => r.host ?? '*')
                    .join(''),
                render: ingress => (_jsx(LabelListItem, { labels: ingress.getRules().map(({ host }) => host || '*') })),
            },
            {
                id: 'ports',
                label: t('translation|Path'),
                getValue: () => '',
                render: (ingress) => _jsx(RulesDisplay, { ingress: ingress }),
            },
            'age',
        ] }));
}
