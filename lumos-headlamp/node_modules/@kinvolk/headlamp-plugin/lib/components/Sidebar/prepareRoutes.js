...
// needed.
function prepareRoutes(t, sidebarToReturn = DefaultSidebars.IN_CLUSTER) {
    ...
    const defaultRoutes = sidebars[DefaultSidebars.IN_CLUSTER];
    for (const route of defaultRoutes) {
        const routeFiltered = filters.length > 0 && filters.filter(f => f(route)).length !== filters.length;
        if (routeFiltered) {
            continue;
        }
        const newSubList = route.subList?.filter(subRoute => !(filters.length > 0 && filters.filter(f => f(subRoute)).length !== filters.length));
