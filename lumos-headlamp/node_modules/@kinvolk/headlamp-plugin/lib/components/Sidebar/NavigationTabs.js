...
function searchNameInSubList(sublist, name) {
    if (!sublist) {
        return false;
    }
...
function findParentOfSubList(list, name) {
    if (!name) {
        return null;
    }
    let parent = null;
...
export default function NavigationTabs() {
    ...
    const tabRoutes = subList
        ...
        .map((item) => {
        return { label: item.label, component: _jsx(_Fragment, {}) };
    });
    defaultIndex = subList.findIndex(item => item.name === sidebar.selected.item);
    return (_jsxs(Box, { mb: 2, component: "nav", "aria-label": t('translation|Main Navigation'), children: [_jsx(Tabs, { tabs: tabRoutes, onTabChanged: index => {
                    tabChangeHandler(index);
                }, defaultIndex: defaultIndex, sx: {
                    maxWidth: '85vw',
                    [theme.breakpoints.down('sm')]: {
                        paddingTop: theme.spacing(1),
                    },
                }, ariaLabel: t('translation|Navigation Tabs') }), _jsx(Divider, {})] }));
}
