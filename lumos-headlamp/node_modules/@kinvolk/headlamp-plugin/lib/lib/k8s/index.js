...
import { clusterRequest } from './apiProxy';
...
export function useCluster() {
    ...
    function getClusterFromLocation() {
        ...
        const clusterURLMatch = matchPath(urlPath, {
            path: getClusterPrefixedPath(),
        });
        return (!!clusterURLMatch && clusterURLMatch.params.cluster) || null;
    }
    const [cluster, setCluster] = React.useState(getClusterFromLocation());
    React.useEffect(() => {
...
export function getVersion(clusterName = '') {
 ...
 */
export function labelSelectorToQuery(labelSelector) {
    const segments = [];
...
export function matchLabelsSimplifier(matchLabels, isEqualSeperator = false) {
    if (!matchLabels) {
        return '';
    }
    const segments = [];
...
export function matchExpressionSimplifier(matchExpressions) {
    if (!matchExpressions) {
        return '';
    }
    const segments = [];
 ...
 */
export function useClustersVersion(clusters) {
    const [clusterNames, setClusterNames] = React.useState(Object.values(clusters).map(c => c.name));
