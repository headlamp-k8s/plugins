 ...
 */
export function useFilterFunc(matchCriteria) {
    const filter = useTypedSelector(state => state.filter);
    return (item, search) => {
        if (item?.metadata) {
            return filterResource(item, filter, search, matchCriteria);
        }
 ...
 */
export function getClusterGroup(returnWhenNoClusters = []) {
    const clusterFromURL = getCluster();
    return clusterFromURL?.split('+') || returnWhenNoClusters;
}
export function useErrorState(dependentSetter) {
 ...
 */
export function flattenClusterListItems(...args) {
 ...
 */
export function combineClusterListErrors(...args) {
 ...
 *
 */
export function useURLState(key, paramsOrDefault) {
    const params = typeof paramsOrDefault === 'object' ? paramsOrDefault : { defaultValue: paramsOrDefault };
...
export function useId(prefix = '') {
    const [id] = React.useState(import.meta.env.UNDER_TEST === 'true'
        ? prefix + 'id'
