 ...
 */
export function filterSources(sources, packageInfos, appMode, compatibleVersion, settingsPackages) {
    ...
    const enabledSourcesAndPackageInfos = sourcesAndPackageInfos.filter(({ packageInfo }) => {
        ...
        const enabledInSettings = settingsPackages[settingsPackages.findIndex(x => x.name === packageInfo.name)]?.isEnabled ===
            true;
        return enabledInSettings;
    });
    const compatible = enabledSourcesAndPackageInfos.filter(({ packageInfo }) => {
        const isCompatible = semver.satisfies(semver.coerce(packageInfo.devDependencies?.['@kinvolk/headlamp-plugin']) || '', compatibleVersion);
        if (!isCompatible) {
 ...
 */
export function updateSettingsPackages(backendPlugins, settingsPlugins) {
    if (backendPlugins.length === 0)
        return [];
    const pluginsChanged = backendPlugins.length !== settingsPlugins.length ||
 ...
 *
 */
export async function fetchAndExecutePlugins(settingsPackages, onSettingsChange, onIncompatible) {
    const pluginPaths = (await fetch(`${helpers.getAppUrl()}plugins`).then(resp => resp.json()));
    const sourcesPromise = Promise.all(pluginPaths.map(path => fetch(`${helpers.getAppUrl()}${path}/main.js`).then(resp => resp.text())));
    const packageInfosPromise = await Promise.all(pluginPaths.map(path => fetch(`${helpers.getAppUrl()}${path}/package.json`).then(resp => {
        if (!resp.ok) {
            if (resp.status !== 404) {
                return Promise.reject(resp);
            }
            {
                console.warn('Missing package.json. ' +
    ...
    const { sourcesToExecute, incompatiblePlugins } = filterSources(sources, packageInfos, helpers.isElectron(), compatibleHeadlampPluginVersion, updatedSettingsPackages);
    if (Object.keys(incompatiblePlugins).length > 0) {
        onIncompatible(incompatiblePlugins);
    }
    const packagesIncompatibleSet = updatedSettingsPackages.map((plugin) => {
        return {
            ...plugin,
