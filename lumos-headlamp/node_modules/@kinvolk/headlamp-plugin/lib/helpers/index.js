 ...
 * ```
 */
export function isDebugVerbose(modName) {
    if (verboseModDebug.filter(mod => modName.indexOf(mod) > 0).length > 0) {
        return true;
    }
    return (import.meta.env.REACT_APP_DEBUG_VERBOSE === 'all' ||
 ...
 * @see isDebugVerbose
 */
export function debugVerbose(modName) {
...
export function getFilterValueByNameFromURL(key, location) {
...
export function addQuery(queryObj, queryParamDefaultObj = {}, history, location, tableName = '') {
 ...
 */
function getAppUrl() {
    let url = exportFunctions.isDevMode() || exportFunctions.isElectron()
        ? 'http://localhost:4466'
 ...
 * @returns void
 */
function setRecentCluster(cluster) {
...
function getRecentClusters() {
    ...
    const recentClusters = JSON.parse(currentClustersStr);
...
function getTablesRowsPerPage(defaultRowsPerPage = 5) {
...
function setTablesRowsPerPage(perPage) {
...
function storeClusterSettings(clusterName, settings) {
    if (!clusterName) {
        return;
    }
    localStorage.setItem(`cluster_settings.${clusterName}`, JSON.stringify(settings));
}
function loadClusterSettings(clusterName) {
    if (!clusterName) {
        return {};
    }
 ...
 */
const backendToken = import.meta.env.REACT_APP_HEADLAMP_BACKEND_TOKEN ||
 ...
 */
export function getHeadlampAPIHeaders() {
