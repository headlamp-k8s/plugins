...
var ze = R((U) => {
  ...
  U.pnpPlugin = void 0;
  var qe = (Me(), W(Ie)), Wt = qe.__importStar(O("fs")), Bt = qe.__importDefault(O("path")), Ht = /()/, Jt = [".tsx", ".ts", ".jsx", ".mjs",
  ".cjs", ".js", ".css", ".json"];
  function Ut(e) {
    return e.map((i) => {
      let t = i.indexOf("*");
...
/`))
  ...
  async function Vt(e) {
    return {
      ...
      resolveDir: Bt.default.dirname(e.path)
    };
  }
  o(Vt, "defaultOnLoad");
  ...
  function Kt({ baseDir: e = process.cwd(), extensions: i = Jt, filter: t = Ht, onResolve: a = Xt, onLoad: r = Vt } = {}) {
    return {
...
function Be(e, i, t = "") {
  ...
  let a = Qt(e), r = 0, n, s;
  for (; r < a.length; r++)
    n = ee(e, a[r]), s = ei(n), s.isDirectory() ? Be(n, i, ee(t, a[r])) : i(ee(t, a[r]), n, s);
}
var Je = q(() => {
  o(Be, "totalist");
});
...
var tt = R((qa, et) => {
  var te = O("fs"), { join: ai, normalize: ri, resolve: ni } = O("path"), { totalist: oi } = (Je(), W(He)), { parse: si } = (Ve(), W(Ge)), {
...
function Hi(e, i) {
  let t = e.path.replace(i, ""), a = Bi(Wi(i, t)), r = `./sb-addons${I(t).split("/").map(encodeURIComponent).join("/")}`;
...
var pa = /^\/($|\?)/, G, C, xt = /* @__PURE__ */ o(async (e) => {
  ...
  return yield, a.use("/", ({ url: P }, S, T) => {
    P && pa.test(P) ? (S.statusCode = 200, S.write(w), S.end()) : T();
  }), a.use("/index.html", (P, S) => {
    S.statusCode = 200, S.write(w), S.end();
  }), {
    bail: fa,
    stats: {
