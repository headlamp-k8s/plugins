...
if (config?.isEnabledInListView === undefined) {
  config = { ...config, isEnabledInListView: true };
}

if (config?.isEnabledInListView) {
  const acceptedList = [
    'headlamp-pods',
    'headlamp-nodes',
    'headlamp-namespaces',
  ...
  const useOpenCostData = () => {
    ...
    return { data, isLoading, setResourceType };
  };
  ...
  function OpenCostData(props: { resource; resourceType }) {
    ...
    const { data, isLoading, setResourceType } = useSharedData();
